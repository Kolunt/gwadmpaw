...
На PythonAnywhere можно настроить автоматическое выполнение периодических задач через встроенный планировщик задач.

### Что делает cron скрипт

Скрипт `cron_tasks.py` выполняет следующие задачи:
...
### Настройка Cron на PythonAnywhere
...
После настройки cron задачи можно проверить её работу:

1. **Проверьте логи выполнения**:
   - В разделе **Tasks** найдите вашу задачу
   - Нажмите на ссылку для просмотра логов выполнения
   - Или проверьте логи через консоль:
     ```bash
     tail -n 50 ~/logs/cron_tasks.log
     ```

2. **Запустите задачу вручную для теста**:
   ```bash
   cd ~/gwadm
   python3.10 cron_tasks.py
   ```

3. **Проверьте результат**:
   - Проверьте базу данных - истекшие коды должны быть очищены
   - Проверьте наличие бэкапов (если включено резервное копирование)

### Настройка дополнительных задач

Если нужно включить дополнительные задачи (очистка логов, бэкапы), отредактируйте файл `cron_tasks.py` и раскомментируйте соответствующие строки:

```python
# Очистка старых логов (раскомментируйте если нужно)
cleanup_old_activity_logs(days=90)

# Резервное копирование базы данных (раскомментируйте если нужно)
backup_database()
```

**Важно**: 
- Убедитесь, что путь к Python правильный (проверьте версию: `python3.10 --version`)
- Убедитесь, что путь к скрипту правильный (`~/gwadm/cron_tasks.py`)
