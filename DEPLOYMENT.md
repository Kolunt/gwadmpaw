...
2. Найдите секцию **Add a new web app** или **Create a new web app**
3. Нажмите на кнопку **Add a new web app** (если у вас еще нет веб-приложения)
4. Если веб-приложение уже создано, пропустите этот шаг и перейдите к Шагу 4

#### Процесс создания:

1. **Выберите домен**:
   - Если у вас бесплатный аккаунт, выберите домен вида `yourusername.pythonanywhere.com`
   - Если у вас оплаченный аккаунт, можете использовать свой домен
   - В нашем случае это `gwadm.pythonanywhere.com`

2. **Выберите Python версию**:
   - Рекомендуется Python 3.10 (или новее, если доступна)
   - Выберите ту же версию, которую вы использовали при установке зависимостей

3. **Нажмите "Next"** для создания веб-приложения

4. **Настройте путь к WSGI файлу**:
   - PythonAnywhere автоматически создаст файл по пути `/var/www/gwadm_pythonanywhere_com_wsgi.py`
   - Этот файл нужно будет отредактировать в следующем шаге
